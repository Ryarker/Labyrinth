...
public class CameraController : MonoBehaviour
{
   ...
   private void Zoom (Touch[] touches) {
        ...
        var perviousDistance = Vector2.Distance(prevPos0, prevPos1);  
        var currentDistance = Vector2.Distance(touches[0].position, touches[1].position);
        var deltaDistance = currentDistance - perviousDistance;
        ...
        }else
        {
            ...
            cam.transform.position -= Vector3.forward * deltaDistance*frustumScale;
            var z = Mathf.Clamp(cam.transform.position.z, 10, 100);
